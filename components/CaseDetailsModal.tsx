...
const CaseDetailsModal: React.FC<CaseDetailsModalProps> = ({ legalCase, onClose, onEdit }) => {
  ...
  const getStatusColor = (status: LegalCase['status']) => {
    switch (status) {
      case 'Ativo': return 'bg-emerald-800 text-white shadow-lg shadow-emerald-950/20';
      ...
      <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-[0_35px_60px_-15px_rgba(0,0,0,0.5)] overflow-hidden animate-in fade-in zoom-in duration-300">
        ...
        <div className="bg-slate-950 px-8 py-6 flex justify-between items-center text-white border-b border-white/5">
          <div className="flex items-center">
            <div className="w-2 h-6 bg-emerald-800 rounded-full mr-4"></div>
            <h2 className="text-xl font-black uppercase tracking-tighter">Detalhes do <span className="text-slate-400">Processo</span></h2>
          </div>
          <div className="flex items-center space-x-3">
            <button 
              onClick={() => onEdit?.(legalCase)}
              className="px-4 py-2 bg-white/10 hover:bg-emerald-800 rounded-xl transition-all flex items-center text-[10px] font-black uppercase tracking-widest"
            >
        ...
        <div className="p-10 max-h-[75vh] overflow-y-auto">
          <div className="flex flex-col md:flex-row justify-between items-start mb-10">
            <div className="mb-4 md:mb-0">
              <p className="text-[10px] font-black text-emerald-800 uppercase tracking-[0.3em] mb-2">Identificador Único</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-10">
            ...
            <div className="space-y-8">
              <div>
                ...
                <div className="flex items-center space-x-4">
                  <div className="bg-slate-100 p-3 rounded-2xl text-emerald-900">
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div>
                ...
                {legalCase.pdfName ? (
                  <button 
                    ...
                    className="flex items-center space-x-4 p-4 bg-emerald-900 text-white rounded-2xl hover:bg-emerald-800 transition-all w-full group shadow-lg border border-emerald-950"
                  >
          ...
          <div className="bg-slate-50 p-8 rounded-[2rem] border border-slate-100 relative overflow-hidden">
             <div className="absolute top-0 left-0 w-1 h-full bg-emerald-800"></div>
